...
void printServerConfig(const ServerConfig& server) {
    ...
    for (size_t i = 0; i < server.serverNames.size(); i++) {
        ...
        if (i < server.serverNames.size() - 1) std::cout << ", ";
    }
    std::cout << std::endl;
    ...
    for (size_t i = 0; i < server.locations.size(); i++) {
        ...
        if (loc.redirect) {
            ...
            std::cout << "      Permanent: " << (loc.redirectPermanent ? "yes" : "no") << std::endl;
        }
        ...
        if (!loc.uploadStore.empty()) {
            ...
            std::cout << "      Max Upload Size: " << loc.maxUploadSize 
                      << " bytes (" << loc.maxUploadSize / SIZE_MB << "MB)" << std::endl;
        }

...
class ClientServer : public IEvenetListeners {
    ...
    public:
        void RegisterWithIOMultiplexer() {
            if (_is_started == true) {
                std::cerr << "WARNING: Attempting to register an already started fd " << _peer_socket_fd << std::endl;
                return;
            }
            ...
            try {
                ...
                LOG_INFO("Client Connected: " + addr + ":" + std::to_string(ntohs(_client_addr.sin_port)) + " Fd " + std::to_string(_peer_socket_fd));
        ...
        ClientServer(const int &server_socket_fd, const int &peer_socket_fd) 
            : _is_started(false), _server_socket_fd(server_socket_fd), _peer_socket_fd(peer_socket_fd) {}
        ...
        virtual void terminate() {
            ...
            std::string addr = inet_ntoa(_client_addr.sin_addr);
            LOG_INFO("Client " + addr + " Fd " + std::to_string(_peer_socket_fd) + " Disconnected!");
            close(_peer_socket_fd);
        }
        ...
        virtual void onEvent(int fd, epoll_event ev) {
            ...
            if (ev.events & EPOLLIN) {
                ...
                if (rd_count < 0) {
                    LOG_ERROR("recv() Failed For Client " + std::to_string(this->_peer_socket_fd));
                    LOG_INFO("Client " + std::to_string(this->_peer_socket_fd) + " Will terminate");
                    this->terminate();
                }
                if (rd_count < RD_SIZE)
